...
Ts = 1/60; %sampling frequency 60Hz

t = 0:Ts:20; % simulation time step
l = 0.1 ; % length robot center to wheel center
d = 0.05; % width robot center to middle of wheel

wMax = pi*2; % maximum body angular velocity rad/s
vMax = 0.8;  % maximum body velocity m/s
noise = 0.0; % Gaussian noise of location tracking (e.g. 0.001)

%state feedback controller
desPoles = [-2-0.5i; -2+0.5i]; % pole placement
...
xRef = cos(freq*t);
yRef = sin(freq*t);
dxRef = -freq*sin(freq*t);
dyRef = freq*cos(freq*t);
ddxRef = -freq^2*cos(freq*t);
...
for k = 1:length(t)
    ...
    q(3) = wrapToPi(q(3)); % wrapToPi = theta % 2*pi 
    v_r(:,k) = v_m;
    q_r(:,k) = q;
    dq_r(:,k) = dq;
    D_r(:,k) = D;
